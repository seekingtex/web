...
const CF_API = (accountId, path) => `https://api.cloudflare.com/client/v4/accounts/${accountId}/${path}`;

const VECTORIZE_UPSERT = (accountId) => CF_API(accountId, `vectorize/v2/indexes/${INDEX_NAME}/upsert`);

const WORKERS_AI_EMBED = (accountId) => CF_API(accountId, 'ai/run/@cf/baai/bge-base-en-v1.5');
...
async function main() {
  ...
  const pages = [
    { id: 'home', url: '/', title: 'seekingtex 鈥?Premium Inflatables', type: 'page', section: 'main' },
    { id: 'about', url: '/about', title: 'About seekingtex', type: 'page', section: 'main' },
    ...
    {
      id: 'product-rib-450-patrol',
      url: '/products/rib-450-patrol',
      title: 'RIB 450 Patrol',
      type: 'product',
      section: 'products',
    },
    {
      id: 'product-airdeck-270',
      url: '/products/airdeck-270',
      title: 'AirDeck 270',
      type: 'product',
      section: 'products',
    },
    {
      id: 'product-airdeck-360',
      url: '/products/airdeck-360',
      title: 'AirDeck 360',
      type: 'product',
      section: 'products',
    },
  ...
  for (const page of pages) {
    ...
    if (!content) {
      console.warn(`  閴?${page.title} 鈥?no built file`);
      continue;
    }

    const chunks = chunkText(content, 400, 50);
    console.log(`  ${page.title} (${page.url}) 閳?${chunks.length} chunks`);
    ...
    const productAttrs = {
      ...
      'product-rib-330': {
        category: 'RIB',
        attributes: { stability: 'high', usage: ['coastal', 'transport', 'commercial'], skill_level: 'advanced' },
        safety: 'life vests required, capacity 4 persons',
      },
      'product-rib-450-patrol': {
        category: 'RIB',
        attributes: { stability: 'high', usage: ['coastal', 'patrol', 'rescue'], skill_level: 'professional' },
        safety: 'life vests required, capacity 6 persons, emergency equipment recommended',
      },
      'product-airdeck-270': {
        category: 'Dinghy',
        attributes: { stability: 'medium', usage: ['lake', 'river', 'fishing'], skill_level: 'beginner' },
        safety: 'life vests recommended, max 3 persons',
      },
      'product-airdeck-360': {
        category: 'Dinghy',
        attributes: { stability: 'medium', usage: ['lake', 'coastal', 'family'], skill_level: 'beginner' },
        safety: 'life vests recommended, max 5 persons',
      },
    ...
    const ssotProducts = {
      'sup-explorer-11': {
        category: 'SUP',
        attributes: { stability: 'high', usage: ['lake', 'sea'], skill_level: 'beginner' },
        safety: 'life vest recommended, leash required',
      },
      'sup-tour-12': {
        category: 'SUP',
        attributes: { stability: 'medium', usage: ['lake', 'sea', 'river'], skill_level: 'intermediate' },
        safety: 'life vest recommended, leash required',
      },
      'kayak-lite': {
        category: 'KAYAK',
        attributes: { stability: 'medium', usage: ['river', 'lake'], skill_level: 'beginner' },
        safety: 'life vest required',
      },
      'kayak-tandem': {
        category: 'KAYAK',
        attributes: { stability: 'medium', usage: ['lake', 'coastal'], skill_level: 'beginner' },
        safety: 'life vest required',
      },
      'life-vest-classic': { category: 'SAFETY', attributes: { usage: ['all'] }, safety: 'CE certified, ISO 12402' },
      'life-vest-pro': {
        category: 'SAFETY',
        attributes: { usage: ['all'] },
        safety: 'CE certified, ISO 12402, SOLAS compliant',
      },
      ...
      try {
        ...
        vectors.push({
          ...
          metadata: {
            ...
            ...(pAttr.attributes && Object.keys(pAttr.attributes).length
              ? { attributes: JSON.stringify(pAttr.attributes) }
              : {}),
...
async function upsertBatch(vectors, accountId, apiToken) {
  ...
  process.stdout.write(`  閴?upserted ${vectors.length} vectors\n`);
}

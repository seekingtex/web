...
const SAMPLE_DATA = [
  {
    query: 'inflatable sup board for beginners',
    page: '/en/guides/beginner-guide',
    impressions: 340,
    clicks: 6,
    ctr: 0.0176,
    position: 11.2,
  ...
  {
    query: 'best paddle board for fishing',
    page: '/en/guides/sup-fishing',
    impressions: 280,
    clicks: 4,
    ctr: 0.0143,
    position: 14.5,
  ...
  {
    query: 'how to choose a paddle board size',
    page: '/en/guides/choosing-paddle',
    impressions: 510,
    clicks: 11,
    ctr: 0.0216,
    position: 9.8,
  ...
  {
    query: 'inflatable vs hard paddle board',
    page: '/en/guides/inflatable-vs-hard',
    impressions: 720,
    clicks: 18,
    ctr: 0.025,
    position: 7.3,
  ...
  {
    query: 'sup board maintenance tips',
    page: '/en/guides/sup-maintenance',
    impressions: 190,
    clicks: 3,
    ctr: 0.0158,
    position: 16.1,
  ...
  {
    query: 'paddle board yoga poses',
    page: '/en/guides/sup-yoga',
    impressions: 145,
    clicks: 2,
    ctr: 0.0138,
    position: 18.4,
  ...
  {
    query: 'best paddle board for kids',
    page: '/en/guides/sup-with-kids',
    impressions: 230,
    clicks: 5,
    ctr: 0.0217,
    position: 12.8,
  ...
  {
    query: 'kayak paddling techniques',
    page: '/en/guides/kayak-techniques',
    impressions: 410,
    clicks: 9,
    ctr: 0.022,
    position: 10.3,
  ...
  {
    query: 'multi day sup trip planning',
    page: '/en/guides/multi-day-trip',
    impressions: 165,
    clicks: 2,
    ctr: 0.0121,
    position: 19.7,
...
function generateSocialPosts(pageSlug) {
  ...
  const hashtags = ['#seekingtex', '#surfing', '#ocean', ...keywords.slice(0, 3).map((k) => `#${k}`)];
  ...
  return [
    ...
    {
      platform: 'tiktok',
      content: `${hook} 馃憜 Full guide at seekingtex.com${url}`,
      hashtags: hashtags.slice(0, 3),
      cta: 'Full guide in bio',
    },
    {
      platform: 'twitter',
      content: `${hook}\n\n${title}\n\nRead 鈫?seekingtex.com${url}\n\n${hashtags.slice(0, 3).join(' ')}`,
      hashtags,
      cta: 'Read full guide',
    },
    {
      platform: 'pinterest',
      content: `${title} u2014 Complete Guide\n\nseekingtex.com${url}`,
      hashtags,
      cta: 'Save for later',
    },
    {
      platform: 'reddit',
      content: `**${title}**\n\nI've been testing different ${keywords.slice(0, 3).join(', ')} in real conditions.\n\n${hook.replace(':', '.')}\n\nFull breakdown: https://seekingtex.com${url}`,
      hashtags: [],
      cta: 'Full breakdown',
    },
  ];
...
function toMarkdown(tasks) {
  ...
  if (tasks.high?.length) {
    ...
    for (const t of tasks.high) lines.push(`- **[${t.type}]** ${t.page} u2014 ${t.action}\n`);
  }
  if (tasks.medium?.length) {
    lines.push('## Medium Priority\n');
    for (const t of tasks.medium) lines.push(`- **[${t.type}]** ${t.page} u2014 ${t.action}\n`);
  }
  return lines.join('\n');
}
...
console.log('=== seekingtex SEO Pipeline v3.6 ===\n');

...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/reindex' && request.method === 'POST') {
      const siteUrl = (env.SITE_URL || '').replace(/\/+$/, '') || 'https://seekingtex.com';
      const sitemapUrl = env.SITEMAP_URL || `${siteUrl}/api/ai/sitemap`;
      ...
      for (const pageMeta of data.pages) {
        const pageRes = await fetch(`${siteUrl}${pageMeta.url}`);

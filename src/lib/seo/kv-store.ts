...
export async function saveTasksToKv(
  env: SeoKvEnv,
  tasks: { high: SeoTask[]; medium: SeoTask[]; low: SeoTask[] }
): Promise<void> {
...
export async function saveHistoryToKv(
  env: SeoKvEnv,
  date: string,
  data: { impressions: number; clicks: number; avgCtr: number; avgPosition: number }
): Promise<void> {
  if (!env.SEO_KV) return;
...
export async function getLatestTasks(
  env: SeoKvEnv
): Promise<{ high: SeoTask[]; medium: SeoTask[]; low: SeoTask[] } | null> {
...
export async function getHistory(
  env: SeoKvEnv,
  date: string
): Promise<{ impressions: number; clicks: number; avgCtr: number; avgPosition: number } | null> {
  if (!env.SEO_KV) return null;

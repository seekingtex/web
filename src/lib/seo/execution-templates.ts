...
const TITLE_PATTERNS: Record<string, string> = {
  ...
  safety: 'Keyword + "What You MUST Know Before Choosing"',
...
export function generateTitleRewrite(entry: ContentV2Record): string | null {
  ...
  if (entry.slug.includes('safety')) return `${clean} (What You MUST Know Before Choosing)`;
  if (entry.category === 'comparison' || entry.slug.includes('vs'))
    return `${clean} (The Honest Answer Nobody Gives You)`;
  ...
  if (entry.slug.includes('brand') || entry.slug.includes('exists'))
    return `${clean} (The Problem With Most Fabric Brands)`;
...
export function generateLinkInjectionPlans(
  entries: ContentV2Record[],
  allEntries: ContentV2Record[]
): LinkInjectionPlan[] {
  ...
  for (const entry of entries) {
    ...
    if (!hasCoreLink && !TIER_A_SLUGS.includes(entry.slug)) {
      plans.push({
        from: entry.slug,
        to: TIER_A_SLUGS[0],
        anchorText: 'how to choose performance fabric for beginners',
...
export const EXECUTION_STEPS: Record<string, string[]> = {
  ...
  LINK_INJECT: [
    'Add link to Tier A core guide (performance fabric, fabric for beginners, or fabric selection guide)',

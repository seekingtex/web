...
export function computeAiCitationScore(
  entries: ContentV2Record[]
): { slug: string; title: string; score: number; grade: string; signals: string[] }[] {
  return entries
    .map((entry) => {
...
export function suggestGeoImprovements(entries: ContentV2Record[]): OptimizationSuggestion[] {
  ...
  for (const entry of entries) {
    ...
    if (score < 60) {
      ...
      if (!entry.tldr || entry.tldr.length < 50)
        suggestion = 'Add concise TLDR block (2-3 sentences) for AI snippet extraction';
      else if (!entry.definition || entry.definition.length < 40)
        suggestion = 'Add definition block with clear term explanation for AI citation';

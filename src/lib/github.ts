...
function getEnv(key: string, fallback: string): string {
  try {
    return ((import.meta.env as Record<string, string | undefined>)[key] as string) || fallback;
  } catch {
    return fallback;
  }
}

const REPO_OWNER = getEnv('GITHUB_OWNER', 'seekingtex');
const REPO_NAME = getEnv('GITHUB_REPO', 'web');
const DEFAULT_BRANCH = 'main';
const COMMIT_AUTHOR_NAME = 'Seekingtex Admin';
const COMMIT_AUTHOR_EMAIL = 'admin@seekingtex.com';
const USER_AGENT = 'Seekingtex-admin';
...
export class GitHubClient {
  ...
  private async request<T>(method: string, path: string, body?: unknown): Promise<T> {
    ...
    if (!res.ok) {
      ...
      let parsed: GitHubApiError | null;
      try {
  ...
  async createFile(filePath: string, content: string, message: string): Promise<CommitResult> {
    ...
    }>('PUT', `/repos/${REPO_OWNER}/${REPO_NAME}/contents/${normalized}`, {
      message,
  ...
  async updateFile(filePath: string, content: string, sha: string, message: string): Promise<CommitResult> {
    const normalized = filePath.replace(/^\/+/, '');
    const encoded = Buffer.from(content, 'utf-8').toString('base64');
    const res = await this.request<{
      commit: { sha: string; html_url: string };

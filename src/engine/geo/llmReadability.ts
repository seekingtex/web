...
function scoreSemanticStructure(p: PageAnalysis): number {
  ...
  if (p.hasArticle) score += 0.2;
  if (p.hasLangAttr) score += 0.1;
...
function scoreMetadata(p: PageAnalysis): number {
  let score = 0;
  if (p.title.length > 0) score += 0.2;
  if (p.metaDescription.length > 0) score += 0.2;
  if (p.hasCanonical) score += 0.15;
  if (p.hasViewport) score += 0.1;
  if (p.jsonLdTypes.length > 0) score += 0.2;
...
function scoreEntityRichness(p: PageAnalysis): number {
  ...
  if (semanticCount >= 5) score += 0.3;
  else if (semanticCount >= 3) score += 0.15;
  if (p.links.filter((l) => l.isInternal).length >= 5) score += 0.2;
...
function scoreLinkHealth(p: PageAnalysis): number {
  ...
  if (internal >= 3) score += 0.3;
  else if (internal >= 1) score += 0.1;
  if (external >= 1) score += 0.2;

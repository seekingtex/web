...
export function generateInsights(scores: GeoScore[]): Insight[] {
  ...
  if (avgScore >= 75) {
    insights.push({
      type: 'positive',
      message: `Average GEO score is ${Math.round(avgScore)} (B or higher)`,
      category: 'overall',
    });
  } else if (avgScore >= 55) {
    insights.push({
      type: 'warning',
      message: `Average GEO score is ${Math.round(avgScore)} — room for improvement`,
      category: 'overall',
    });
  } else {
    insights.push({
      type: 'critical',
      message: `Average GEO score is ${Math.round(avgScore)} — needs significant improvement`,
      category: 'overall',
    });
  }

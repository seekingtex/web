...
const RULES: { name: string; weight: number; fn: (p: PageAnalysis) => { passed: boolean; detail: string } }[] = [
  {
    ...
    fn: (p) => ({
      ...
      detail: p.title ? `Title: "${p.title.slice(0, 60)}"` : 'Missing <title>',
    }),
  },
  {
    name: 'title-length',
  ...
  },
  {
    ...
    weight: 8,
    fn: (p) => ({
      passed: p.metaDescription.length > 0,
      detail: p.metaDescription ? 'Has meta description' : 'Missing meta description',
    }),
  },
  {
    ...
    weight: 4,
    fn: (p) => ({
      passed: p.metaDescription.length >= 50 && p.metaDescription.length <= 160,
      detail: `Meta desc length: ${p.metaDescription.length}`,
    }),
  },
  {
    name: 'has-h1',
    ...
    fn: (p) => {
      ...
      return {
        passed: h1.length === 1,
        detail: h1.length === 1 ? `One H1: "${h1[0].text.slice(0, 50)}"` : `H1 count: ${h1.length}`,
      };
    },
  },
  {
  ...
  },
  {
    ...
    weight: 5,
    fn: (p) => ({
      passed: 'description' in p.ogTags,
      detail: 'description' in p.ogTags ? 'Has og:description' : 'Missing og:description',
    }),
  },
  {
    name: 'og-image',

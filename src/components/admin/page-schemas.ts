...
export const PAGES: PageSpec[] = (pagesData as PageSpec[]).length > 0 ? (pagesData as PageSpec[]) : [];

export const PAGE_BY_SLUG: Record<string, PageSpec> = Object.fromEntries(PAGES.map((p) => [p.slug, p]));

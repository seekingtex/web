...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const systemPrompt = `You are Seekingtex Product Selection Engine.
...
- [any conditions to avoid]`;
    ...
    if (!llmQuota.allowed) {
      return new Response(
        JSON.stringify({ error: 'Daily AI quota exceeded', remaining: llmQuota.remaining, limit: DAILY_NEURON_LIMIT }),
        {
          ...
          headers: {
            'Content-Type': 'application/json; charset=utf-8',
            ...quotaResponseHeaders(llmQuota.remaining, DAILY_NEURON_LIMIT),
          },
        }
      );
    }
    ...
    return new Response(JSON.stringify({ answer: llmRes.response }), {
      status: 200,
      headers: {
        'Content-Type': 'application/json; charset=utf-8',
        ...quotaResponseHeaders(llmQuota.remaining, DAILY_NEURON_LIMIT),

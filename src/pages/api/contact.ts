...
async function loadBranding(env?: Record<string, unknown>): Promise<BrandingEmailConfig> {
  ...
  return {
    ...
    contact_email_provider:
      fromEnv.contact_email_provider ||
      secretsData.contact_email_provider ||
      brandingData.contact_email_provider ||
      'none',
    contact_from_email:
      fromEnv.contact_from_email || secretsData.contact_from_email || brandingData.contact_from_email || '',
    contact_from_name:
...
export const POST: APIRoute = async ({ request, clientAddress, locals }) => {
  try {
    ...
    if (
      branding.contact_email_provider === 'resend' &&
      branding.contact_resend_api_key &&
      branding.contact_from_email
    ) {
    ...
    return jsonResponse(
      { success: false, error: 'Internal server error: ' + (e instanceof Error ? e.message : 'unknown') },
      500
    );
  }
};

...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  for (const c of req.changes) {
    if (!c || typeof c.path !== 'string' || typeof c.sha !== 'string')
      return errorResponse('Each change must contain path and sha', 400);
    ...
    try {
      ...
      if (file.sha !== change.sha) {
        results.push({
          path: change.path,
          ok: false,
          error: 'File modified by another user, please refresh and retry',
        });
        continue;
      }

...
function configTemplate(locale: string, name: string, _dir: string): string {
  const upper = locale.toUpperCase();
  void name;
  return `site:
  name: Seekingtex
  site: 'https://seekingtex.com'
  base: '/'
  trailingSlash: false

metadata:
  title:
    default: Seekingtex — {name}
    template: '%s — Seekingtex'
  description: 'Seekingtex — {name}'
  keywords:
    - Seekingtex
  author: Seekingtex
  robots:
...
function navTemplate(_name: string): string {
  return `header:
  topBar:
    - text: Find a Store
      href: /store-locator
  ...
  links:
    - text: Products
      links:
        - text: All Products
          href: /products
        - text: Sun Protection Fabrics
          href: /sun-protection-fabric
        - text: Waterproof Fabrics
          href: /waterproof-fabric
        - text: Accessories
          href: /accessories
    - text: Resources
      links:
...
footer:
  ...
  socialLinks:
    ...
    - ariaLabel: GitHub
      ...
      href: https://github.com/seekingtex
...
async function readLocaleList(
  client: import('~/lib/github').GitHubClient
): Promise<Array<{ code: string; name: string; locale: string; textDirection: string; enabled: boolean }>> {
...
async function syncCiWorkflow(client: import('~/lib/github').GitHubClient, locales: string[]): Promise<void> {
  ...
  try {
    ...
    const updated = existing.content.replace(/^\s+locale:\s*\[.+\]/m, localeArray);
...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    ...
    const yamlMod = await import('js-yaml');
    const newYamlContent = yamlMod.dump(existingList, {
      lineWidth: 120,
      noRefs: true,
      sortKeys: false,
      quotingType: "'",
    ...
    const updatedList = [
      ...existingList.filter((l) => l.locale !== code),
      { code: code.toUpperCase(), name, locale: code, textDirection },
    ];
    updatedList.sort((a, b) => a.locale.localeCompare(b.locale));
    await syncCiWorkflow(
      client,
      updatedList.map((l) => l.locale)
    );
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      ...
      await client.updateFile(
        'src/data/site/languages.yaml',
        newContent,
        sha,
        `chore: remove ${code} from languages.yaml`
      );
    }
  } catch (e) {

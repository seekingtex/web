...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (!quota.allowed) {
    return new Response(JSON.stringify({ error: 'Daily AI quota exceeded', ...quota }), {
      ...
      headers: {
        ...
        ...quotaResponseHeaders(quota.remaining, DAILY_NEURON_LIMIT),
      },
    });
  }
  ...
  try {
    ...
    if (!embedConsume.allowed) {
      return new Response(
        JSON.stringify({
          ...
          remaining: embedConsume.remaining,
        ...
        }),
        {
          ...
          headers: {
            'Content-Type': 'application/json; charset=utf-8',
            ...quotaResponseHeaders(embedConsume.remaining, DAILY_NEURON_LIMIT),
          },
        }
      );
    }
    ...
    const prompt = `You are a search assistant for Seekingtex.
...
Provide a concise answer with references to the sources when appropriate.`;
    ...
    if (!llmConsume.allowed) {
      return new Response(
        JSON.stringify({
          error: 'Daily AI quota exceeded',
          remaining: llmConsume.remaining,
          limit: DAILY_NEURON_LIMIT,
        }),
        {
          status: 429,
          headers: {
            'Content-Type': 'application/json; charset=utf-8',
            ...quotaResponseHeaders(llmConsume.remaining, DAILY_NEURON_LIMIT),
          },
        }
      );
    }

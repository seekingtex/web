...
const publicPaths = ['/login', '/api/auth/login', '/api/auth/logout', '/api/auth/change-password'];
...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  const isProtected = protectedPrefixes.some(
    (p) => url === p || url.startsWith(p + '/') || (p.endsWith('/') && url.startsWith(p))
  );
  ...
  if (!shouldSkipI18n(url)) {
    ...
    if (hasLangPrefix && pathLang === defaultLang) {
      const base = url.split('?')[0];
      if (base === `/${defaultLang}` || base === `/${defaultLang}/`) {
        const qs = url.includes('?') ? url.slice(url.indexOf('?')) : '';
        return context.redirect('/' + qs, 308);
      }
    }
